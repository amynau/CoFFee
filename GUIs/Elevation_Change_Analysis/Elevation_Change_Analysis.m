...
[file_name, path_name] = uigetfile({'*.asc';'*.tif'},'select file for DEM 1');
...
[file_name, path_name] = uigetfile({'*.asc';'*.tif'},'select file for DEM 2');
...
[file_name, path_name] = uigetfile({'*.asc';'*.tif'},'select file for UNC 1');
...
[file_name, path_name] = uigetfile({'*.asc';'*.tif'},'select file for UNC 2');
...
% prompt for file
[file_name, path_name] = uigetfile({'*.txt';'*.shp'},'select file for polygon');
...
txt = get(handles.txt_out,'String');
DefaultName = [txt{1} '.txt'];
DefaultName = regexprep(DefaultName,':','-');

% dialog box to prompt saving
[FileName,PathName,FilterIndex] = uiputfile('*.txt','Save file name',DefaultName);
